...
| `disabled` | no              | `false`             | Set to `true` to exclude this metric from collection and analysis             |
...
Set `disabled: true` on a metric override to skip its Prometheus query and omit it
from the report for that workload. Only `name` and `disabled` are needed when
...
`metrics_yaml_path` can explicitly set `disabled: false` to re-enable a metric and
