...
metrics:
  - name: my_metric           # Unique identifier (required)
    ...
    disabled: false           # Set true to exclude from collection and analysis
...
| `disabled` | no | `false` | Set to `true` to exclude the metric from collection and analysis for this workload |
...
- A metric with `disabled: true` is removed after all configuration layers are merged
- A higher-priority layer can set `disabled: false` to re-enable a metric and reuse its inherited query
