...
export const defaultMetrics: Array<Metric> = [
	...
	{
		id: 'read_throughput',
		query: `sum by(ref) (rate(sdk_operations_total{operation_type="read"}[2s]) > 0)`,
		description: 'Read operations throughput',
	},
	{
		id: 'write_throughput',
		query: `sum by(ref) (rate(sdk_operations_total{operation_type="write"}[2s]) > 0)`,
		description: 'Write operations throughput',
	},
	{
		id: 'read_attempts',
		query: `sum by(ref) (rate(sdk_retry_attempts{operation_type="read"}[2s]) > 0)`,
		description: 'Read attempts throughput',
	},
	{
		id: 'write_attempts',
		query: `sum by(ref) (rate(sdk_retry_attempts{operation_type="write"}[2s]) > 0)`,
