...
async function main() {
    ...
    for (let workload of Object.keys(pulls)) {
        ...
        let { data: { artifacts }, } = await getOctokit(token).rest.actions.listWorkflowRunArtifacts({
            owner: context.repo.owner,
            repo: context.repo.repo,
            run_id: latestRun.id,
        });
        ...
        if (!artifact || artifact.expired) {
            ...
            downloadPath ?? (downloadPath = path.join(cwd, artifact.name));
    ...
    for (let workload of Object.keys(metrics)) {
        ...
        let report = await renderReport(metrics[workload]);
        debug(`Report: ${report}`);
