...
function drawLegend(ctx, width, height, legend) {
    ctx.font = '14px Arial';
    ctx.fillStyle = '#000';
    ...
    legend.forEach((item, index) => {
        // Draw color box
        ctx.fillStyle = COLORS[item.toLowerCase()];
        ctx.fillRect(legendX, legendY + index * 20, 20, 2);
        // Draw label
        ctx.fillStyle = '#000';
        ctx.fillText(item, legendX + 30, legendY + 10 + index * 20);
    });
}
export async function generateChart(title, series, xLabel, yLabel, legend) {
    ...
    drawLegend(ctx, WIDTH, HEIGHT, legend);
    // Draw data lines
    series.forEach((s, i) => {
