...
export async function getCurrentWorkflowRuns(branch: string) {
	let {
		data: { workflows },
	} = await getOctokit(getInput('github_token', { required: true })).rest.actions.listRepoWorkflows({
	...
	let {
		data: { workflow_runs },
	} = await getOctokit(getInput('github_token', { required: true })).rest.actions.listWorkflowRuns({
		owner: context.repo.owner,
		repo: context.repo.repo,
